...
from Business.text import subscription_title_generator, lead_customer_on_shopping, successful_buy_notification
from Payment.models import iTTransfer
from Platform.external_services.sms_ir import send_message
...
def buy_new_customer_v1abcd12(_, d, device):
    if "Validate Request and Initialize":
        try:
            cache = Cache.g(token__exact=d['key'])
            ...
            validate_fullname = fullname is None or len(fullname) > 5

            validate = cache and business and validate_mobile and validate_fullname and not device
            if not validate:
                raise ValueError
    ...
    if "Looking For Customer Account":
        customer = User.find_user(mobile)

    is_new_customer = False
    if "Create New Customer" and not customer:
        customer = User()
        customer.save()
    ...
    if "Check For Loyal Customers":
        if is_new_customer:
            order.purchase_number = 1
        else:
            order.purchase_number = Order.f(business__exact=business, customer__exact=customer).count() + 1
    ...
    if "Done":
        ...
        if "Send Message For Customer":
            is_new_customer = cache.data['is_new_customer']
            parameters, template_id = lead_customer_on_shopping(customer, is_new_customer, business, cash_amount)
            send_message(customer.mobile, parameters, template_id)
        ...
        return {"order": order.buyer_api_v2(), "state": cache.data['is_new_customer']}  # @todo
