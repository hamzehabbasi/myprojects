...
@usage
def edit_business_abcd1234(_, d, device):
    if 'Validate Request and Initialize':
        try:
            ...
            validate_business_data = 3 <= len(title) <= 50 and len(address) > 7
            validate_licence = len(license_number) == 10 and license_expire >= datetime.now() + timedelta(days=7)

            validate = validate_business_data and validate_licence
    ...
    if 'Update Business':
        ...
        business.city_ilink = city_ilink
        business.licence_number = license_number
        business.licence_expire = license_date
        business.save()
