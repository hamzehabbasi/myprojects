...
from Platform.models import User, Prompt, ID
...
@usage
def start_public_business_abcd1234(_, d, device):
    if 'Validate Request and Initialize':
        try:
            ...
            mobile_validate = len(mobile) == 11 and mobile.startswith('09')
            fullname_validate = len(fullname) > 5

            if not Business.k(demo_key):
                raise ValueError

            if not mobile_validate or not fullname_validate:
                raise ValueError
