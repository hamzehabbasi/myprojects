...
def seller_api_v2(self):
    ...
    return {
        ...
        "off_budget_consumed": self.budget_consumed,
...
@usage
def add_off_v2abcd12(req, d, device):
    if 'Validate Request and Initialize':
        ...
        try:
            ...
            if business.offs.A().count() > business.subscription.config['limitation']['offs']:
                return 429

            code = None
            if d['code'] and d['code'] != "":
                code = d['code'].upper()
                codes = [x.code for x in business.offs.A()]
                if code in codes:
                    return 409
            ...
            title = d['title']
            off_type = d['off_type']
            affect_type = d['affect_type'] if 'affect_type' in d.keys() else 'direct'
