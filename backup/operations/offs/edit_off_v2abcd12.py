...
@usage
def edit_off_v2abcd12(req, d, device):
    if 'Validate Request and Initialize':
        ...
        try:
            ...
            if d['code'] and d['code'] != "":
                code = d['code'].upper()
                codes = [x.code for x in business.offs.A().exclude(key__exact=off.key)]
                if code in codes:
                    return 409
    ...
    if "Check For Discount CODE" and config['features']['offDiscountCode']:
        if not off.data['off_budget_consumed']:
            off.code = code
