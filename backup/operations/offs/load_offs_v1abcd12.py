...
from Business.models import Off, iTSubscription
...
@usage
def load_offs_v2abcd12(_, d, device):
    if 'Partial Fetch':
        try:
            keys = d['keys']
    ...
    offs = Off.objects.none()

    if 'Partial Fetch' and keys:
        offs = Off.af(business__customer__exact=device.user).af(key__in=d['keys'])

    if 'Full Fetch' and not keys:
        offs = Off.f(business__customer__exact=device.user)

    offs = offs.prefetch_related(
        'for_certain_products',
        'for_certain_services',
        # 'for_certain_customer_groups',
        'for_certain_customers',
        # 'for_certain_categories',
    )

    return [x.seller_api_v2() for x in offs.order_by('update')]
