...
from Business.models import BusinessProductLabel, VitrineCategory
...
@usage
def edit_product_dcba4321(_, d, device):
    if 'Validate Request and Initialize':
        try:
            ...
            validate = 50 >= len(name) > 2 and len(unit) > 0 and price > 0 and order in [15, 50, 100] and category
            if not validate:
                raise ValueError
